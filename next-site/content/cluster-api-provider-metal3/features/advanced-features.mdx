...
- [Node Reuse](/cluster-api-provider-metal3/features/node-reuse) — 節點重複利用機制
- [Label 同步](/cluster-api-provider-metal3/features/labelsync) — BMH 與 Node 標籤同步
...
- 🟡 **中階工程師**：重點關注 ClusterClass 整合與多叢集隔離的設計思路
...
→ ClusterClass 引入了 **topology 控制器**，它會動態 patch 由 ClusterClass 產生的物件。若直接修改由 ClusterClass 產生的 Metal3Cluster/Metal3Machine，修改可能被 topology 控制器覆蓋。需透過 ClusterClass variables 或 patches 正式修改。
...
**誤解 3：「watchAllNamespaces=true 在小型叢集中效能影響不大」**
→ 監控所有 namespace 會讓 controller 的 informer 快取大幅增長，在有大量 namespace 的叢集中可能導致記憶體壓力。除非有明確需求（如多租戶共用管理叢集），否則建議保持預設值。

**誤解 4：「MachinePool 和 MachineDeployment 功能相同，只是命名不同」**
→ 兩者架構不同。MachineDeployment 是 1:1 管理（一個 Machine 對應一台裸機），MachinePool 是批量管理模式，適合同質化的大規模節點群。Metal3 的 MachinePool 支援仍處於實驗性階段。
