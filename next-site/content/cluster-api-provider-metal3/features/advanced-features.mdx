...
MachinePool 的設計初衷是讓一個物件代表多台機器，適合動態擴縮的場景，但bare-metal環境的特性（機器需要明確 provisioning）使得這個功能在 Metal3 中的語義較為複雜。
...
Metal3 支援 **ARM64** 和 **x86_64** 兩種架構的bare-metal，只需在 `Metal3MachineTemplate` 中指定對應架構的 OS image 即可：
...
→ 兩者架構不同。MachineDeployment 是 1:1 管理（一個 Machine 對應一台bare-metal），MachinePool 是批量管理模式，適合同質化的大規模node群。Metal3 的 MachinePool 支援仍處於實驗性階段。
