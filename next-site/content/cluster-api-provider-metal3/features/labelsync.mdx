...
- 🟢 **初學者**：了解「為什麼需要把硬體資訊傳到 K8s Node 上」是本頁核心，先看「行李箱標籤」類比
...
<Callout type="info" title="概念類比：行李箱的硬體規格標籤">

想像你在管理一個機場行李系統：

- **BMH** = 每個行李箱（每台裸機伺服器）
- **BMH labels** = 貼在行李箱上的硬體標籤（「內含 GPU」、「高速 NIC」、「機架 A3」）
- **LabelSync** = 行李資訊同步系統，自動把行李箱的標籤資訊傳到托運系統（K8s Node）
- **K8s Node labels** = 托運系統中的記錄（讓排程系統知道「這台機器有 GPU」）

沒有 LabelSync，應用程式（Pod）就不知道哪台節點有 GPU，只能隨機排程，可能把 AI 工作負載排到沒有 GPU 的機器上。
