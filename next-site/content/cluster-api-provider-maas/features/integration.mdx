...
本指南說明如何在既有環境中部署 MAAS Provider，並從零開始建立第一個bare-metal Kubernetes cluster。

## 前置需求

### Management Cluster 需求

在開始之前，您需要一個已運行的 Kubernetes cluster作為 **management cluster**：
...
| OS Images | 已匯入 MAAS 的 OS image，例如 Ubuntu 22.04 LTS 搭配 Kubernetes 元件 |
| 機器狀態 | 機器已完成 **Enlist → Commission**，狀態為 `Ready`，等待被部署 |


<Callout type="warning" title="MAAS API Key 安全性">

MAAS API key 具有完整的機器控制權（包含部署、刪除、重開機）。請**妥善保管**並使用 Kubernetes Secret 儲存，**絕對不要**直接寫入 MaasCluster 或 MaasMachine 等 CRD spec 中。一旦洩漏，攻擊者可以透過 MAAS API 任意操控所有bare-metal 機器。
...
以下範例建立一個 3 control plane node的 HA cluster，使用 MAAS 作為 infrastructure provider。
...
等待cluster就緒後，取得 workload cluster 的 kubeconfig：

```bash
# 等待cluster就緒（ControlPlaneReady = true）
...
<Callout type="warning" title="CNI 未安裝時cluster無法正常運作">

CAPI 建立的cluster**預設沒有 CNI**，所有node會處於 `NotReady` 狀態，Pod 無法跨node通訊。必須在cluster建立後手動安裝 CNI 才能讓cluster正常運作。
...
kubectl --kubeconfig my-maas-cluster.kubeconfig apply \
  -f https://docs.tigera.io/calico/latest/manifests/calico.yaml
```
...
使用 `MachineDeployment` 管理 worker node群組：
...
| Node 不加入cluster（NotReady）| CNI 未安裝或 cloud-init userdata 錯誤 | 查看 kubelet 日誌：`journalctl -u kubelet -f` |
...
# 使用 clusterctl 顯示cluster整體狀態（含 control plane、machine deployment）
clusterctl describe cluster my-maas-cluster
```

`clusterctl describe cluster` 會以樹狀結構顯示cluster中所有 CAPI 物件的狀態，是排查問題時最快速的全局視圖工具。
