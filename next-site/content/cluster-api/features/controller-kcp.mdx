...
- [KubeadmControlPlane API](/cluster-api/features/api-kubeadm-controlplane)
...
- 🟢 **初學者**：建議先閱讀 KubeadmControlPlane API 頁面，再來理解 KCP Controller 的運作細節
- 🟡 **中階工程師**：重點掌握 Preflight Checks 機制，了解 KCP 在什麼條件下才允許 scale/rollout
- 🔴 **資深工程師**：深入研究 etcd quorum 保護設計與 Rolling Update 演算法中的機器選擇策略
</Callout>

> ⚙️ **類比**：KCP Controller 就像一位「嚴格遵守作業程序的資深工程師」— 在操作任何高風險動作（scale up/down、rollout）前，他必須逐項確認安全清單（preflight checks），確保 etcd 委員會成員數量充足（quorum），系統各元件都健康，才肯動手。而且他每次只做一個動作（一次一個操作原則），完成後才考慮下一步。

## KCP Controller 概覽

`KubeadmControlPlaneReconciler`（位於 `controlplane/kubeadm/internal/controllers/controller.go`）是 CAPI 中負責管理 Control Plane node的核心controller。其主要職責包括：

- **管理 Control Plane node生命週期**：建立、更新與刪除 CP Machine
- **協調資源關係**：`KubeadmControlPlane` → `Machine` → `KubeadmConfig` + `InfrastructureMachineTemplate`
- **確保 etcd cluster健康**：於 scale/rollout 前後驗證 etcd quorum
...
`Reconcile()` 函式是controller的入口點，負責串接整個協調流程：
...
為尚未設定 `cluster.x-k8s.io/machine-certificates-expiry-date` annotation 的 `KubeadmConfig` 查詢並記錄各 CP node的 API Server 憑證到期日。
...
| `rollout.after` scheduled to期 | `ShouldRolloutAfter(reconciliationTime, kcp.Spec.Rollout.After)(machine)` | `filters.go` |


<Callout type="info" title="Labels/Annotations 不觸發 Rollout">

KCP 設計上，Machine 的 labels 和 annotations 變更**不會觸發 Rolling Update**。這些欄位是 in-place mutable（透過 `syncMachines()` 直接同步到現有 Machine），以避免不必要的node重建。
...
- `rollout.after` 觸發（scheduled rollout，需要重建）

只有 `EligibleForInPlaceUpdate = true` 且啟用 `InPlaceUpdates` feature gate 時，KCP 才會嘗試 in-place update 而非重建 Machine。

---

<Callout type="tip" title="新手提示：一次一個操作原則">
KCP 的「一次一個操作（One Operation at a Time）」原則是確保 Control Plane 穩定性的關鍵。每次 Reconcile 循環中，KCP 只會執行以下其中一個操作：建立新 CP Machine、刪除舊 CP Machine、或執行 in-place 更新。這樣即使操作失敗，影響範圍也限定在單台機器，不會造成多台 CP 同時不健康。這和一般應用服務可以批量更新的做法完全不同，因為 Control Plane 承擔整個cluster的管理功能，任何操作都必須格外謹慎。
</Callout>
...
Machine 設定 DeletionTimestamp
  ...
  → KCP reconcilePreTerminateHook() 處理
      ...
      ├─ RemoveEtcdMember()：從 etcd cluster移除此 member
      └─ removePreTerminateHookAnnotationFromMachine()：解除刪除阻塞
```


<Callout type="warning" title="只處理一台機器">
...
kubeconfig Secret 的命名規則為 `<cluster-name>-kubeconfig`，存放在管理cluster（management cluster）的同一 Namespace 下。
...
<Callout type="info" title="kubeconfig 用途">
...
這是重要的安全設計：避免在 CA 遺失時意外產生新 CA，導致整個cluster憑證體系崩潰。若初始化後 CA Secret 遺失，KCP 會設定 `CertificatesAvailableCondition = False` 並停止操作。

### reconcileCertificateExpiries() — 到期日追蹤

此函式查詢 workload cluster 中各node實際的 API Server 憑證（`/etc/kubernetes/pki/apiserver.crt`）到期日：
...
`Rollout.Before.CertificatesExpiryDays` 預設值讓 KCP 在憑證到期前 **N 天**觸發 rolling update。由於 certificate rollout 等同於完整的 Rolling Update，KCP 會依序重建所有 Control Plane Machine，讓 kubeadm 在每台node上重新產生並簽發憑證。


<Callout type="warning" title="憑證到期風險">
...
KCP 可以**採用**（adopt）已存在但沒有 ownerReference 的 CP Machine，例如在手動建立 Machine 或在cluster遷移場景中。
...
<Callout type="warning" title="採用後立即 Requeue">
...
1. **等待 Worker 先刪除**：若cluster中還有非 CP 的 Machine 或 MachinePool，KCP 等待它們先被刪除（`RequeueAfter: 15s`）
...
刪除時**不進行** etcd member 的逐一清理，因為整個 etcd cluster都會被刪除。
...
<Callout type="tip" title="Provider Contract 的延伸設計">
...
<Callout type="info" title="暫停行為說明">
...
> 💡 **思考一下**：KCP 在 Rolling Update 中採用「先 Scale Up 再 Scale Down」的策略（先建立新 CP node，確認健康後才刪除舊node）。這個策略在 etcd quorum 的視角下有什麼特別考量？如果 replicas=3，升級過程中cluster短暫有 4 台 CP node，etcd 如何處理這個「過渡狀態」？
...
1. **「KCP Controller 直接控制 kubeadm 執行」** — 不正確。KCP Controller 透過建立 Machine 物件來間接觸發node的 bootstrap 流程，`kubeadm init/join` 是由 Bootstrap Provider（CABPK）產生的 cloud-init 腳本在node上自動執行，KCP 本身並不直接 SSH 進機器操作。
2. **「Preflight 檢查失敗後 KCP 會無限重試」** — 不正確。KCP 在 Preflight 失敗時會 requeue（加上延遲重試），但如果是結構性問題（如 etcd quorum 長期無法恢復），需要人工介入修復，KCP 不會強行跳過 Preflight 繼續執行。
3. **「KCP 的憑證輪替會導致服務中斷」** — 通常不會。憑證輪替（Certificate Rotation）只是重新產生並替換 Secret，API Server 重啟期間有短暫不可用，但 etcd 和其他組件不受影響，client 端通常在 retry 後即可恢復連線。
</Callout>
...
- [KubeadmControlPlane API](/cluster-api/features/api-kubeadm-controlplane) — KCP Spec 欄位與 RemediationStrategy 設定
- [CAPI 架構概覽](/cluster-api/features/architecture) — Control Plane Provider 合約介面
- [Bootstrap KubeadmConfig](/cluster-api/features/bootstrap-kubeadmconfig) — kubeadm init/join 設定詳解
