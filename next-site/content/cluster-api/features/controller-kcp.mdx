...
KCP 的「一次一個操作（One Operation at a Time）」原則是確保 Control Plane 穩定性的關鍵。每次 Reconcile 循環中，KCP 只會執行以下其中一個操作：建立新 CP Machine、刪除舊 CP Machine、或執行 in-place 更新。這樣即使操作失敗，影響範圍也限定在單台機器，不會造成多台 CP 同時不健康。這和一般應用服務可以批量更新的做法完全不同，因為 Control Plane 承擔整個叢集的管理功能，任何操作都必須格外謹慎。
</Callout>
...
> 💡 **思考一下**：KCP 在 Rolling Update 中採用「先 Scale Up 再 Scale Down」的策略（先建立新 CP 節點，確認健康後才刪除舊節點）。這個策略在 etcd quorum 的視角下有什麼特別考量？如果 replicas=3，升級過程中叢集短暫有 4 台 CP 節點，etcd 如何處理這個「過渡狀態」？
...
1. **「KCP Controller 直接控制 kubeadm 執行」** — 不正確。KCP Controller 透過建立 Machine 物件來間接觸發節點的 bootstrap 流程，`kubeadm init/join` 是由 Bootstrap Provider（CABPK）產生的 cloud-init 腳本在節點上自動執行，KCP 本身並不直接 SSH 進機器操作。
