...
- 🟢 **初學者**：了解為什麼需要 ClusterResourceSet，以及它如何解決「cluster建好後還要手動裝 CNI」的痛點
...
> - 每個新cluster（新員工）帶上特定 label（部門label）時
...
傳統上，新cluster建立完成後，工程師需要手動安裝 CNI 插件、StorageClass、監控 agent 等元件。CRS 解決了這個問題：只要cluster帶有特定 label，CAPI controller就會自動將對應的 addon 資源部署進去。
...
| 欄位 | 類型 | 必要 | 說明 |
|------|------|------|------|
| `spec.clusterSelector` | LabelSelector | ✅ | 選擇要套用資源的cluster |
...
| 2 | 建立 ClusterResourceSet，選擇器匹配 `cni: calico` 的cluster |
...
在使用 ClusterClass 建立cluster時，可以在 Cluster 物件的 `metadata.labels` 中預先設定好對應的 addon label。如此一來，只要cluster透過 ClusterClass 建立完成，CRS controller便會立即偵測到符合條件的cluster並自動套用 addon，實現**「建立cluster即自動安裝 addon」**的完整自動化流程，無需任何手動介入。
...
- 若 workload cluster 的 API Server 不可用，CRS controller會持續重試，直到套用成功為止
