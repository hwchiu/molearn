...
- 🔴 **資深工程師**：深入研究 Provider 合約不可變性設計與 ClusterCache 連線管理架構
</Callout>
...
> 💡 **思考一下**：如果你要實作一個新的 Infrastructure Provider（例如支援某家雲端廠商），根據 CAPI 的 Provider 合約，你至少需要實作哪些 CRD？這些 CRD 的哪些 Status 欄位是 CAPI 核心 Controller 必須依賴的？
...
1. **「CAPI 能直接管理雲端資源（如 AWS EC2）」** — 錯誤。CAPI Core 只負責協調邏輯，實際建立雲端資源的是對應的 Infrastructure Provider（如 CAPA）。CAPI 透過「合約」介面與 Provider 溝通，本身不與任何雲端 API 互動。
2. **「ClusterCache 會快取所有 workload cluster 的物件」** — 不完全正確。ClusterCache 只為每個 workload cluster 建立一個帶有 informer 的 client，快取的範圍取決於哪些 controller 實際去 List/Watch 哪些資源，並非快取所有物件。
3. **「Webhook 失效只影響新資源的建立」** — 不正確。Conversion Webhook 失效時，即使是查詢現有資源（使用不同 API 版本）也會失敗，因為 Kubernetes 每次回傳物件前都需要執行版本轉換。
</Callout>
...
- [Core Controller 詳解](/cluster-api/features/controller-core) — ClusterReconciler / MachineReconciler 的完整 Reconcile 流程
- [KCP Controller 詳解](/cluster-api/features/controller-kcp) — KubeadmControlPlane Controller 的 Preflight 與 Rolling Update 演算法
- [Topology Controller 詳解](/cluster-api/features/controller-topology) — ClusterClass 的 Template Reconcile 流程
- [Cluster 與 Machine API](/cluster-api/features/api-cluster-machine) — 核心資源的 Spec/Status 欄位說明
