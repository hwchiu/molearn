---
layout: doc
title: Cluster API — clusterctl 操作指南
---

# clusterctl 操作指南


<Callout type="info" title="相關章節">

- [架構概覽](/cluster-api/features/architecture)
- [Machine 生命週期](/cluster-api/features/machine-lifecycle)
- [ClusterClass 與 Topology](/cluster-api/features/clusterclass-topology)

</Callout>

<Callout type="tip" title="本文適用對象">
- 🟢 **初學者**：從 `clusterctl init` 開始，這是安裝 CAPI Provider 的第一步
- 🟡 **中階工程師**：重點掌握 `clusterctl move`（pivot 流程）與升級 Provider 的操作順序
- 🔴 **資深工程師**：深入研究離線（air-gapped）安裝方式與自定義 Provider URL 設定
</Callout>

> 🎛️ **類比**：clusterctl 是 CAPI 的「維運駕駛艙」— 就像飛機駕駛艙中有各種儀表板和控制桿，`kubectl` 負責操控工作cluster內部的應用程式，而 `clusterctl` 則是操控cluster本身的生命週期。安裝 Provider、建立cluster、搬移管理權、升級版本，這些「cluster級別的操作」都透過 clusterctl 這個駕駛艙來執行，讓維運人員有清晰的控制界面。

## clusterctl 是什麼

`clusterctl` 是 Cluster API（CAPI）的官方 CLI 工具，負責管理整個 CAPI 生態系的核心操作：

- **Provider 管理**：安裝、升級、移除 Infrastructure / Bootstrap / ControlPlane Provider
- **工作cluster生命週期**：建立、升級、刪除、搬移 workload cluster
- **cluster診斷**：提供cluster狀態樹狀視圖與除錯功能

### 安裝方式

從 GitHub Release 下載二進位檔：

```bash
# 下載最新版本（Linux）
curl -L https://github.com/kubernetes-sigs/cluster-api/releases/latest/download/clusterctl-linux-amd64 -o clusterctl
chmod +x clusterctl
sudo mv clusterctl /usr/local/bin/
```

---

## clusterctl init — 安裝 Provider

`clusterctl init` 用於在 management cluster 上安裝 CAPI Provider。

```bash
# 安裝 CAPI core + Metal3 Provider
clusterctl init \
  --infrastructure metal3 \
  --bootstrap kubeadm \
  --control-plane kubeadm

# 安裝特定版本
clusterctl init \
  --infrastructure metal3:v1.6.0 \
  --bootstrap kubeadm:v1.6.0 \
  --control-plane kubeadm:v1.6.0

# 安裝 MAAS Provider
clusterctl init \
  --infrastructure maas:v1.0.0 \
  --bootstrap kubeadm \
  --control-plane kubeadm
```

### init 執行的步驟

1. 讀取 clusterctl 設定（`~/.cluster-api/clusterctl.yaml`）
2. 下載 Provider 的 manifest（從 GitHub Release 或自定義 URL）
3. 套用 CRD 和 RBAC 資源
4. 部署 Provider controller（Deployment）
5. 等待 Provider 就緒

### clusterctl.yaml 設定範例

若需使用自定義 Provider URL，可在設定檔中指定：

```yaml
# ~/.cluster-api/clusterctl.yaml
providers:
  - name: metal3
    url: https://github.com/metal3-io/cluster-api-provider-metal3/releases/download/v1.6.0/infrastructure-components.yaml
    type: InfrastructureProvider
  - name: maas
    url: https://github.com/spectrocloud/cluster-api-provider-maas/releases/download/v1.0.0/infrastructure-components.yaml
    type: InfrastructureProvider
```

---

## clusterctl generate cluster — 產生cluster YAML

`clusterctl generate cluster` 根據 Provider 提供的模板產生cluster所需的 YAML 資源。

```bash
# 產生 Metal3 cluster YAML
clusterctl generate cluster my-cluster \
  --infrastructure metal3 \
  --kubernetes-version v1.28.0 \
  --control-plane-machine-count 3 \
  --worker-machine-count 5 \
  > my-cluster.yaml

# 使用自定義 flavor
clusterctl generate cluster my-cluster \
  --infrastructure metal3:v1.6.0 \
  --flavor without-kube-vip \
  --kubernetes-version v1.28.0 \
  > my-cluster.yaml
```

### 環境變數替換

產生 YAML 時，clusterctl 會自動替換模板中的環境變數。使用前需先設定必要的環境變數：

```bash
export CLUSTER_NAME=my-cluster
export NAMESPACE=metal3
export KUBERNETES_VERSION=v1.28.0
export CONTROL_PLANE_MACHINE_COUNT=3
export WORKER_MACHINE_COUNT=5
# Metal3 特有變數
export IMAGE_URL=https://images.example.com/ubuntu-22.04.tar.gz
export IMAGE_CHECKSUM=sha256:abcdef...

clusterctl generate cluster my-cluster --infrastructure metal3 | kubectl apply -f -
```

若要查看某個 Provider 模板需要哪些變數，可使用 `--list-variables`：

```bash
clusterctl generate cluster my-cluster \
  --infrastructure metal3 \
  --list-variables
```

---

<Callout type="tip" title="新手提示：clusterctl vs kubectl 的使用場景">
`kubectl` 操作的是 Kubernetes 資源物件（Pod、Service、Deployment），`clusterctl` 操作的是 CAPI 的管理層面（Provider 安裝、cluster搬移、Provider 升級）。平時運維 workload cluster 的應用程式用 `kubectl`；安裝或升級 CAPI 本身、搬移管理cluster等「cluster基礎設施管理」操作才使用 `clusterctl`。
</Callout>

## clusterctl describe cluster — 查看cluster狀態

`clusterctl describe cluster` 以樹狀結構呈現cluster內所有 CAPI 資源的狀態，是診斷問題的第一步。

```bash
# 查看cluster狀態
clusterctl describe cluster my-cluster -n metal3

# 輸出範例：
# NAME                                                          READY  SEVERITY  REASON  SINCE  MESSAGE
# Cluster/my-cluster                                            True                     5m
# ├─ClusterInfrastructure - Metal3Cluster/my-cluster           True                     5m
# ├─ControlPlane - KubeadmControlPlane/my-cluster-cp           True                     5m
# │ └─3 Machines...
# │   ├─Machine/my-cluster-cp-abc12                           True                     4m
# │   ├─Machine/my-cluster-cp-def34                           True                     4m
# │   └─Machine/my-cluster-cp-ghi56                           True                     4m
# └─Workers
#   └─MachineDeployment/my-cluster-workers                    True                     4m
#     └─5 Machines...
```

### 常用選項

```bash
clusterctl describe cluster my-cluster \
  --namespace metal3 \
  --show-conditions all \
  --grouping false
```

| 選項 | 說明 |
|------|------|
| `--show-conditions all` | 顯示所有 Condition（包含 True 狀態） |
| `--grouping false` | 不分組顯示 Machine，逐一列出 |
| `--all-namespaces` | 列出所有 namespace 中的cluster |

---

## clusterctl get kubeconfig — 取得 kubeconfig

取得 workload cluster 的管理員 kubeconfig，讓你能直接操作 workload cluster：

```bash
# 取得 workload cluster 的 kubeconfig
clusterctl get kubeconfig my-cluster -n metal3 > my-cluster.kubeconfig

# 驗證連線
kubectl --kubeconfig=my-cluster.kubeconfig get nodes
```

---

## clusterctl upgrade — 升級 Provider

`clusterctl upgrade` 用於規劃並執行 CAPI Provider 的版本升級。

### 查看可用升級

```bash
clusterctl upgrade plan

# 輸出範例：
# Latest release available for the v1beta1 API Version of Cluster API (contract):
#
# NAME                    NAMESPACE     TYPE                     CURRENT VERSION   NEXT VERSION
# bootstrap-kubeadm       capi-system   BootstrapProvider        v1.5.0            v1.6.0
# control-plane-kubeadm   capi-system   ControlPlaneProvider     v1.5.0            v1.6.0
# cluster-api             capi-system   CoreProvider             v1.5.0            v1.6.0
# infrastructure-metal3   capm3-system  InfrastructureProvider   v1.5.0            v1.6.0
```

### 執行升級

```bash
clusterctl upgrade apply \
  --contract v1beta1 \
  --core capi-system/cluster-api:v1.6.0 \
  --bootstrap capi-system/kubeadm:v1.6.0 \
  --control-plane capi-system/kubeadm:v1.6.0 \
  --infrastructure capm3-system/metal3:v1.6.0
```


<Callout type="warning" title="升級前注意事項">

升級 Provider 前，建議先備份 management cluster 的 etcd。Provider 升級通常不影響 workload cluster，但需確認新版本的 Provider 與現有 workload cluster 版本相容。

</Callout>


---

## clusterctl move — 搬移cluster

`clusterctl move` 用於將 management cluster 中的 CAPI 資源搬移至另一個cluster，常見於 pivot 流程。

```bash
# 將資源從 bootstrap cluster 搬到 management cluster（pivot）
clusterctl move \
  --to-kubeconfig=management-cluster.kubeconfig \
  --namespace metal3

# 搬移特定cluster
clusterctl move \
  --to-kubeconfig=new-management.kubeconfig \
  --namespace metal3 \
  --filter-cluster my-cluster
```

### move 的適用場景

| 場景 | 說明 |
|------|------|
| Bootstrap → 永久 management cluster | 初始部署完成後的 pivot 操作 |
| Management cluster 遷移 | 將管理權轉移至新cluster |
| 災難復原 | 從備份重建 management cluster 後還原管理資源 |


<Callout type="info" title="move 操作前置條件">

move 操作前，確保目標cluster已安裝相同版本的 Provider（`clusterctl init`），且 workload cluster 正常運行。move 只搬移管理資源，不影響 workload cluster 本身的運行。

</Callout>


---

## 常用診斷指令

**查看已安裝 Provider 狀態：**

```bash
clusterctl describe provider -n capi-system
```

**列出所有cluster（跨 namespace）：**

```bash
clusterctl describe cluster --all-namespaces
```

**查看 Provider 模板所需的環境變數：**

```bash
clusterctl generate cluster my-cluster \
  --infrastructure metal3 \
  --list-variables
```

> 💡 **思考一下**：`clusterctl move` 在搬移cluster管理權時，為什麼需要「先 Pause 來源cluster、複製資源到目標cluster、再 Resume」這樣的三段式操作？如果省略 Pause 步驟直接複製資源，可能發生什麼問題？

---

## 常見誤解

<Callout type="warning" title="常見誤解澄清">
1. **「clusterctl init 只需要執行一次」** — 不完全正確。每次新增 Provider 或升級 Provider 版本都需要再次執行（`clusterctl upgrade apply` 或重新 `clusterctl init`）。此外，在 air-gapped 環境中還需要額外設定 custom image repository。
2. **「clusterctl generate cluster 會直接建立cluster」** — 錯誤。`clusterctl generate cluster` 只是**產生 YAML manifest**，並輸出到 stdout。你還需要把這份 YAML 透過 `kubectl apply` 套用到 management cluster，才會真正觸發cluster建立流程。
3. **「clusterctl move 可以在任何時候執行」** — 不正確。`clusterctl move` 有前置條件：目標 management cluster 必須已安裝相同的 CAPI Provider，且版本相容。另外，如果cluster處於升級進行中狀態時搬移，可能造成 reconcile 狀態不一致。
</Callout>

---

## 延伸閱讀

- [CAPI 架構概覽](/cluster-api/features/architecture) — CAPI Provider 的種類與設計
- [ClusterClass 與 Topology](/cluster-api/features/clusterclass-topology) — 使用 `clusterctl generate cluster --topology` 產生 Topology cluster
- [Controller Core 詳解](/cluster-api/features/controller-core) — management cluster 中各 Controller 的運作方式
